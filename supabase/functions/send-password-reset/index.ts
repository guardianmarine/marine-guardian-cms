...
serve(async (req) => {
  ...
  try {
    ...
    const redirectTo = `${req.headers.get("origin") || supabaseUrl}/auth/callback?type=recovery`;
    console.log("Generating password reset for:", targetUser.email, "with redirect:", redirectTo);
    ...
    console.log("Password reset link generated for:", targetUser.email);
