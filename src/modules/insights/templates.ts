...
export const insightTemplates: Record<string, {
  ...
  top_combos: {
    ...
    async run({ dateFrom, dateTo, limit, locale }) {
      try {
        // Fetch deals in range (READ-ONLY query)
        const { data: deals, error: e1 } = await supabase
          ...
          .or('status.eq.issued,status.eq.partially_paid,status.eq.paid');
        
        if (e1) {
          throw new Error(e1.message || fmtExp(
        ...
        if (e2) {
          throw new Error(e2.message || fmtExp(
            'Failed to load deal units',
            'Error al cargar unidades de ventas',
            locale
          ));
        }
        ...
        if (e3) {
          throw new Error(e3.message || fmtExp(
            'Failed to load units data',
            'Error al cargar datos de unidades',
            locale
          ));
        }
        ...
        console.error('Error in top_combos:', error);
        throw new Error(error.message || fmtExp(
          'Failed to load top combos. Please check your database connection and permissions.',
          'Error al cargar mejores combos. Por favor verifica tu conexión y permisos de base de datos.',
          locale
        ));
      }
    }
  ...
  slow_movers: {
    ...
    async run({ dateFrom, dateTo, limit, locale }) {
      try {
        ...
        console.error('Error in slow_movers:', error);
        throw new Error(error.message || fmtExp(
          'Failed to load slow movers. Please check your database connection and permissions.',
          'Error al cargar inventario lento. Por favor verifica tu conexión y permisos de base de datos.',
          locale
        ));
      }
    }
      ...
      try {
        ...
        if (error) {
          throw new Error(error.message || fmtExp(
            ...
            'Error al cargar datos de oportunidades',
            locale
          ));
        }
        ...
        console.error('Error in lost_reasons:', error);
        throw new Error(error.message || fmtExp(
          'Failed to load lost reasons. Please check your database connection and permissions.',
          'Error al cargar razones de pérdida. Por favor verifica tu conexión y permisos de base de datos.',
          locale
        ));
      }
    }
      ...
      try {
        ...
        if (e2) {
          throw new Error(e2.message || fmtExp(
            ...
            'Error al cargar datos de cuentas',
            locale
          ));
        }
        ...
        console.error('Error in repeat_buyers:', error);
        throw new Error(error.message || fmtExp(
          'Failed to load repeat buyers. Please check your database connection and permissions.',
          'Error al cargar compradores recurrentes. Por favor verifica tu conexión y permisos de base de datos.',
          locale
        ));
      }
    }
  ...
  rep_performance: {
    ...
    async run({ dateFrom, dateTo, limit, locale }) {
      try {
        ...
        console.error('Error in rep_performance:', error);
        throw new Error(error.message || fmtExp(
          'Failed to load rep performance. Please check your database connection and permissions.',
          'Error al cargar rendimiento de vendedores. Por favor verifica tu conexión y permisos de base de datos.',
          locale
        ));
      }
    }
