-- Migration: Fix convert_buyer_request_to_lead RPC - Schema Alignment
-- Description: Corrects RPC to match REAL schema (accounts.kind/is_active, contacts.first_name/last_name, lead_id)

-- ============================================================================
-- DROP AND RECREATE RPC WITH CORRECT SCHEMA
-- ============================================================================
...
'Converts a buyer_request into an account (kind/is_active), contact (first_name/last_name), and lead. Returns lead UUID.';
...
BEGIN
  ...
  IF v_function_exists THEN
    ...
    RAISE NOTICE '   ✓ Uses accounts.kind (not type)';
    RAISE NOTICE '   ✓ Uses accounts.is_active (not active)';
    RAISE NOTICE '   ✓ Accounts has no email/phone fields';
    RAISE NOTICE '   ✓ Uses contacts.first_name/last_name (not name)';
    RAISE NOTICE '   ✓ Uses buyer_requests.lead_id (not converted_to_lead_id)';
    RAISE NOTICE '   ✓ Auth check: raises exception if auth.uid() is NULL';
  ELSE
    RAISE WARNING '⚠️  Function not found after recreation';
  END IF;
END $$;
